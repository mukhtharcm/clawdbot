...
export const telegramUserPlugin: ChannelPlugin<ResolvedTelegramUserAccount> = {
  ...
  config: {
    ...
    isConfigured: (account) => Boolean(account.credentials.apiId && account.credentials.apiHash),
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (
        resolveTelegramUserAccount({ cfg: cfg as CoreConfig, accountId }).config.allowFrom ?? []
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      return {
        ...
        normalizeEntry: (raw) => raw.replace(/^(telegram-user|telegram|tg):/i, "").toLowerCase(),
      };
    },
  ...
  outbound: {
    deliveryMode: "direct",
    chunker: (text, limit) => getTelegramUserRuntime().channel.text.chunkMarkdownText(text, limit),
