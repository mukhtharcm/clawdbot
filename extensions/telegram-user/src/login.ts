...
export async function loginTelegramUser(params: {
  ...
  const passwordPrompt = passwordEnv ? passwordEnv : async () => await promptText("2FA password: ");
        ...
        ? {
            ...
            invalidCodeCallback: async (type) => {
              ...
              if (type === "code" && codeEnv) {
                runtime.error?.("Telegram code rejected. Update TELEGRAM_USER_CODE and rerun.");
              }
            },
          }
        : {
