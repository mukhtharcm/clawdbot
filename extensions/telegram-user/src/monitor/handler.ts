...
/**
 * Build parent peer for forum topic binding inheritance.
 * Topic peers include a `:topic:<id>` suffix, so we provide the base group peer
 * for fallback matching when bindings are configured at the group level.
 */
export function buildTelegramUserParentPeer(params: {
  isGroup: boolean;
...
function resolveTelegramUserGroupConfig(
  ...
  const topicConfig = groupConfig?.topics?.[topicKey] ?? groups["*"]?.topics?.[topicKey];
  return { groupConfig, topicConfig };
}

function extractTelegramUserLocation(media: MessageContext["media"]): NormalizedLocation | null {
...
export function createTelegramUserMessageHandler(params: TelegramUserHandlerParams) {
  ...
  return async (msg: MessageContext) => {
    ...
    try {
      ...
      const isGroup = msg.chat.type === "chat" && msg.chat.chatType !== "channel";
      ...
      const threadId =
        isGroup && isForum && isTopicMessage
      ...
      if (isDirect) {
        ...
        const groupPolicy = accountConfig.groupPolicy ?? defaultGroupPolicy ?? "allowlist";
      ...
      const primaryMessage = messageGroup.find((entry) => entry.text?.trim()) ?? msg;
      ...
      const replyToSender = replyInfo?.sender ? describeReplySender(replyInfo.sender) : undefined;
      ...
      const groupPeerId =
        isGroup && chatId != null ? buildTelegramUserGroupPeerId(chatId, threadId) : null;
      const parentPeer =
        isGroup && chatId != null
          ? buildTelegramUserParentPeer({ isGroup, chatId, threadId })
          : undefined;
      ...
      const replySenderId =
        msg.replyToMessage?.sender?.type === "user" ? msg.replyToMessage.sender.id : undefined;
      ...
      const mediaSuffix = !rawBody && allMedia.length > 1 ? ` (${allMedia.length} items)` : "";
      ...
      const ctxPayload = core.channel.reply.finalizeInboundContext({
        ...
        MediaTypes:
          allMedia.length > 0
            ? (allMedia.map((item) => item.contentType).filter(Boolean) as string[])
            : undefined,
