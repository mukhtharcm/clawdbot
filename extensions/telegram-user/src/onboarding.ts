...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  getCurrent: (cfg) => (cfg as CoreConfig).channels?.["telegram-user"]?.dmPolicy ?? "pairing",
...
export const telegramUserOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  getStatus: async ({ cfg }) => {
    ...
    return {
      channel,
      configured,
      statusLines: [`Telegram User: ${configured ? "configured" : "needs API ID + API hash"}`],
  ...
  configure: async ({
    ...
    const canUseEnv = resolvedAccountId === DEFAULT_ACCOUNT_ID && Boolean(envApiId && envApiHash);
